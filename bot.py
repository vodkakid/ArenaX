...
def main():
    ...
    # ── Capture de disputa — ANTES de comp_conv ────────────────────────────────
    ...
    tournament_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(admin.create_tournament_start, pattern="^admin_tournament_create$")
        ],
        states={
            admin.TOURN_NAME:  [
    ...
    # ── Broadcast + Límite victorias — UN SOLO handler ────────────────────────
    admin_input_conv = ConversationHandler(
        ...
        states={
            admin.ADMIN_TEXT_INPUT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin.handle_admin_text_input)
            ],
            admin.BROADCAST_OK: [
    ...
    manage_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(admin.manage_player_search, pattern="^admin_manage_player$")
        ],
        states={
            admin.MANAGE_SEARCH: [
    ...
    # Callbacks obsoletos — SIEMPRE AL FINAL
    app.add_handler(CallbackQueryHandler(common.handle_stale_callback))

    setup_jobs(app)
    logger.info("ArenaX Bot v6.2 iniciado ✅")
    app.run_polling(
